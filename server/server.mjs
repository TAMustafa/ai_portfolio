...
function tokenize(text) {
  return (text || '').toLowerCase()
    ...
    .split(/\s+/)
    .filter((t) => t && t.length >= 2 && !['the','and','or','about','tell','me','you','are','what','do'].includes(t));
}
...
function scoreDoc(query, doc) {
  ...
  for (const tok of tokens) {
    ...
    const bodyHits = (body.match(new RegExp(tok.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g')) || []).length;
...
function retrieveTopSnippets(query, lang, k = 3) {
  ...
  if(ranked.length > 0) console.log('  -> Top snippet:', ranked[0].title, `(Score: ${ranked[0].score})`);
  console.log(`--------------------`);
  return ranked;
}
...
app.post('/api/chat', async (req, res) => {
  ...
  const knowledgeBlock = snippets
    .map((s, i) => `(#${i + 1}) [${s.lang}] ${s.title} (/${s.lang}/project/${s.slug})\nTags: ${s.tags.join(', ')}\nExcerpt: ${s.excerpt}`)
    .join('\n\n');
